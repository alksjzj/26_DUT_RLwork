...
class MazeRenderer:
    def __init__(self, maze_generator):
        ...
        self.root = tk.Tk()
        self.root.title(f"迷宫 (Tkinter版) - Seed: {MY_SEED}")
        self.root.resizable(False, False)
        
        self.canvas = tk.Canvas(self.root, width=self.width, height=self.height, 
                                bg=self._rgb_to_hex(BLUE),
    ...
    def draw(self):
        ...
        for r in range(self.maze.rows):
            for c in range(self.maze.cols):
                cell_value = self.maze.grid[r][c]
                
                # 如果是墙(1)或者是背景色，我们可以选择不画(透出Canvas背景)或者画黑色
                color = None
                if cell_value == 1:
                    color = BLACK
                elif cell_value == 0:
                ...
                if color: 
                    ...
                    self.canvas.create_rectangle(x1, y1, x2, y2, fill=self._rgb_to_hex(color), outline="")

        # 绘制起点
        sx, sy = self.maze.start_pos
        self.canvas.create_rectangle(
            sx*CELL_SIZE, sy*CELL_SIZE, (sx+1)*CELL_SIZE, (sy+1)*CELL_SIZE, 
            fill=self._rgb_to_hex(GREEN), outline=""
        )

        # 绘制终点
        ex, ey = self.maze.end_pos
        self.canvas.create_rectangle(
            ex*CELL_SIZE, ey*CELL_SIZE, (ex+1)*CELL_SIZE, (ey+1)*CELL_SIZE, 
            fill=self._rgb_to_hex(RED), outline=""
        )
...
if __name__ == "__main__":
    ...
    maze = MazeGenerator(width_cells=71, height_cells=71)
